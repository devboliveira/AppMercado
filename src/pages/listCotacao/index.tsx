...
export default function ListCotacao() {
  ...
  async function handleOption(option: string) {
    ...
    if (option === "Balanco") {
      ...
      await AsyncStorage.setItem("@selectedCotacao", JSON.stringify(selectedId));
      navigation.navigate('Cotacao');
    }
    closeModal();
  }
          ...
          renderItem={({ item }) => {
            ...
            return (
              ...
              >
                ...
                <Text style={{ color: "#666" }}>Criado em: {item.created_at} {item.status === 'ATIVO' ? "" : "Finalizado em: " + item.data_finalizacao }</Text>
              </TouchableOpacity>
            );
          }}
        />
